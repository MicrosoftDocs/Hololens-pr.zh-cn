...
### 自动眼部位置支持
- 现在，我们通过自动眼部位置支持为全息影像定位提供更高的准确度，从而提高观看舒适度并改善显示质量。 

在 HoloLens 2 中，眼部位置可实现准确的全息影像定位、舒适的观看体验和改进的显示质量。 眼部位置作为眼球跟踪结果的一部分计算。 但是，这需要每个用户进行眼球跟踪校准，即使在体验不需要眼部凝视输入的情况下也是如此。

**自动眼部位置 (AEP)** 使这些场景能够以无交互方式为用户眼部位置。  从用户戴上设备开始，“自动眼部位置”将自动在后台开始工作。 如果用户没有之前的眼球跟踪校准，自动眼部位置将在一小段处理时间后开始向显示系统提供用户的眼部位置。 此处理时间通常介于 20-60 秒之间。 用户数据不会保留在设备上，因此，如果用户取下并重新戴上设备，或者设备重新启动或从睡眠状态唤醒，则会重复此过程。  

当未经校准的用户戴上设备时，“自动眼部位置”功能会改变一些系统行为。 未校准的用户指的是之前未在设备上完成眼球跟踪校准过程的人员。

|     活动应用程序                           |     旧行为                                   |     Windows HWindow 全息版 20H2 及以后版本的行为                                                     |
|--------------------------------------------------|--------------------------------------------------------|------------------------------------------------------------------------------------------------------------|
|     未启用凝视的应用或全息外壳    |     将显示眼球跟踪校准提示。    |     不显示提示。                                                                                |
|     已启用凝视的应用                             |     将显示眼球跟踪校准提示。    |     只有当应用程序访问眼部凝视流时，才会显示眼球跟踪校准提示。     |

 如果用户从未启用凝视的应用程序转换为访问凝视数据的应用程序，则会显示校准提示。 将不会对开箱即用体验流进行更改。 
 
对于需要眼部凝视数据或非常精确的全息影像定位的体验，我们建议未校准的用户从眼球跟踪校准提示或者从“开始”菜单启动“设置”应用，然后选择“**系统”>“校准”>“眼部校准”>“运行眼部校准**”，来运行眼球跟踪校准。

**已知问题**
 - 我们正在调查一个问题，即当在内存负载较重的情况下运行时，眼球跟踪器驱动程序主机进程可能会崩溃。 眼球跟踪驱动程序主机进程应自动恢复。
