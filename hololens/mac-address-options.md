...
| 解决方案 | 优势 | 要求 |
| --- | --- | --- |
| 使用以太网适配器预配程序包 | 改进 OOBE 体验，更快地提供技术人员体验。 | HoloLens 兼容的 USB-C Hub + 适配器和技术支持人员仍然需要与用于 MAC 捕获和 OOBE 完成的设备进行交互 |
...
- 可访问客户网络的有线网络端口
- HoloLens 兼容的 USB-C Hub 与适配器 - 任何无需其他驱动程序或应用程序安装的适配器应该合适。
- 预配程序包包括：
  - 包含无线网络信息和证书
...
通过此操作可"轻触"设备，应用正确的预配包并收集设备的 MAC 地址。 [可按此指南创建预配程序包。](https://docs.microsoft.com/hololens/hololens-provisioning)
...
需要更多先决条件，如下所示：
1. [为 Autopi preview 应用程序启用租户](https://docs.microsoft.com/hololens/hololens2-autopilot)。
...
3. 设备应在 OOBE 期间通过以太网适配器自动连接到 Internet。 它应检测 Autopi detect 配置，并自动向 Azure AD 和 Intune 注册。

4. 设备将按需要通过 Intune 应用所需的 Wi-Fi 证书和其他配置。

5. 完成后，技术人员可以加载 Intune (Endpoint Manager) 门户，并深入了解 **Home -> Devices -> DeviceName -> Hardware**中的设备属性页。
...
这将为技术员提供"头灯"部署体验，设备无需使用设备即可在 Azure AD 和 Intune 中注册设备，无需使用设备或与 HoloLens 环境手动进行交互。
