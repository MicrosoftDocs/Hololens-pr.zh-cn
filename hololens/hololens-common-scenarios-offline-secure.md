...
本指南提供应用示例预配包的指南，该预配包将锁定 HoloLens 2，以在具有以下限制的安全环境中使用：
-   禁用 WiFi。
-   禁用蓝牙。
-   禁用麦克风。
-   阻止添加或删除预配包。
...
Windows 10 电脑设置
1. [将最新的 HoloLens 2 操作系统文件直接](https://aka.ms/hololens2download) 下载到电脑。 
   1. 内部版本 19041.1117 及以上版本中包含对此配置的支持。
1. 将高级恢复助手 (ARC) 工具 [从 Microsoft Store](https://www.microsoft.com/store/productId/9P74Z35SFRS8) 下载到电脑
1. 将最新的 Windows 配置设计器 ([WCD) ](https://www.microsoft.com/p/windows-configuration-designer/9nblggh4tx22?activetab=pivot:overviewtab) 从 Microsoft Store 下载到电脑。
1. [下载OfflineSecureHL2_Sample文件生成](https://aka.ms/HoloLensDocs-SecureOfflineSample) PPKG 的文件文件夹。
...
1. 在电脑上启动 WCD 工具。
1. 选择 **"文件>打开项目。**
  1. 导航到之前保存的OfflineSecureHL2_Sample的位置，然后选择：OfflineSecureHL2_Sample.icdproj.xml
1. 项目应打开，现在应该具有可用自定义项列表： 
...
|     项目                                                |     设置                       |     描述                                                                                                                    |
|---------------------------------------------------------|-----------------------------------|------------------------------------------------------------------------------------------------------------------------------------|
|     帐户/用户                                    |     本地用户名&密码    |     对于这些脱机设备，需要由设备的所有用户设置和共享单个用户名和密码。          |
|     First Experience / HoloLens / SkipCalibration       |     True                          |     仅在初始设备设置期间跳过校准                                                                             |
|     First Experience / HoloLens / SkipTraining          |     True                          |     在初始设备设置期间跳过设备培训                                                                              |
|     First Experience / HoloLens / WiFi                  |     True                          |     在Wi-Fi安装期间跳过配置                                                                                 |
|     Policies/Connectivity/AllowBluetooth                |     否                            |     禁用Bluetooth                                                                                                             |
|     策略/体验/AllowCortana                    |     否                            |     禁用 Cortana (消除潜在的问题，因为麦克风在)                                           |
|     Policies/MixedReality/MicrophoneDisabled            |     是                           |     禁用麦克风                                                                                                            |
|     Policies/Privacy/LetAppsAccessLocation              |     强制拒绝                    |     阻止应用尝试访问位置 (消除潜在的问题，因为禁用了位置跟踪)     |
...
4. 在"运行时设置"下 **，选择帐户/用户/用户名：Holo/Password** 
    - 记下密码并重置（如果需要）。
5. 导航到 UniversalAppInstall /UserContextApp，并配置你要部署到这些设备的 [LOB](app-deploy-provisioning-package.md) 应用。

   > [!div class="mx-imgBorder"]
   > ![在 WCD 中添加应用位置的屏幕截图。](images/offline-secure-sample-wcd-usercontextapp.png)

6. 完成后，选择"导出"按钮并按照所有提示操作，直到创建预配包。
...
1. 设备完成安装和启动备份后，从电脑导航到文件资源管理器，将之前保存的 PPKG 文件复制到设备文件夹。

   > [!div class="mx-imgBorder"]
   > ![文件资源管理器窗口中电脑上的 PPKG 文件。](images/offline-secure-file-explorer.png)

1. 在 HoloLens 2 上，按以下按钮组合以运行预配包 **：同时点击** 音量降低和 **电源** 按钮。
1. 系统将提示你应用预配包，选择"确认 **"**
1. 设置包完成后，选择"**确定"。**
...
通过此配置，建议重新启动上述过程，使用 ARC 工具重新将设备放大并应用新的 PPKG，以对操作系统和/或应用程序进行 (更新) 。 
