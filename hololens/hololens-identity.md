...
与其他 Windows 设备一样，HoloLens 始终在用户上下文中运行。 始终存在用户标识。 HoloLens 处理标识的方式与其他 Windows 设备的处理方式几乎相同。 本文对 HoloLens 上的标识进行深入了解，重点介绍 HoloLens 与其他 Windows 设备的不同之处。

HoloLens 支持多种用户标识。 你可以使用一个或多个用户帐户登录。 下面是有关 HoloLens 上的标识类型和身份验证选项的概述：

| 标识类型 | 每台设备的帐户 | 身份验证选项 |
| --- | --- | --- |
| [Azure Active Directory](/azure/active-directory/)<sup>1</sup>  | 64 | <ul><li>Azure web 凭据提供程序</li><li>Azure Authenticator 应用</li><li>生物识别 (Iris) &ndash; 仅 HoloLens 2<sup>2</sup> </li><li>FIDO2 安全密钥</li><li>&ndash;为 HoloLens (第一代) 提供可选的 PIN HoloLens 2</li><li>密码</li></ul> |
...
> [!IMPORTANT]
> 3-只能在 [OOBE 期间通过预配包](hololens-provisioning.md#apply-a-provisioning-package-to-hololens-during-setup)在设备上设置本地帐户，而不能稍后在 "设置" 应用中添加该帐户。 如果要在已设置的设备上使用本地帐户，则需要 [刷新或重置设备。](hololens-recovery.md)

## <a name="setting-up-users"></a>设置用户

可以通过两种方式在 HoloLens 上设置新用户。 最常见的方法是 (OOBE) HoloLens 全新体验。 如果使用 Azure Active Directory，则[其他用户可以](#setting-up-multi-user-support-azure-ad-only)使用其 Azure AD 凭据在 OOBE 后登录。 在 OOBE 期间，最初设置为 MSA 或本地帐户的 HoloLens 设备将不支持多个用户。 请参阅[ (第一代) ](hololens1-start.md)或[HoloLens 2](hololens2-start.md)设置 HoloLens。
...
HoloLens 支持来自同一 Azure AD 租户的多个用户。 若要使用此功能，你必须使用属于你的组织的帐户来设置设备。 随后，同一租户中的其他用户可以通过登录屏幕或通过点击 "开始" 面板上的 "用户" 磁贴登录到该设备。 一次只能有一个用户登录。 当用户登录时，HoloLens 注销上一个用户。 

>[!IMPORTANT]
> 设备上的第一个用户被视为设备所有者，但在 Azure AD 联接的情况下，请 [详细了解设备所有者](security-adminless-os.md#device-owner)。
...
用于开发 HoloLens 不同于桌面开发的一种方法是不完全支持[OnlineIDAuthenticator](/uwp/api/Windows.Security.Authentication.OnlineId.OnlineIdAuthenticator) API。 尽管 API 在主帐户正常运行时返回令牌，但本文中所述的中断不会为用户显示任何 UI，并且无法正确验证帐户。
...
HoloLens 2 Iris 身份验证。 Iris 基于Windows Hello技术，并且支持由 Azure Active Directory 和 Microsoft 帐户使用。 Iris 的实现方式与其他技术Windows Hello相同，并实现[1/100K 生物识别安全](/windows/security/identity-protection/hello-for-business/hello-biometrics-in-enterprise#has-microsoft-set-any-device-requirements-for-windows-hello)。
