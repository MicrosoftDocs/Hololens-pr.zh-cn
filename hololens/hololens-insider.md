...
本文介绍改进的故障排除和设备报告、展台模式下的一些修复 bug 和证书查看器、扩展的可管理性图面以及更新可靠性的提升。 此功能更新的一项新功能即将HoloLens移动平台模式。 查看所有新的出色的功能，HoloLens 2！
...
✔️ 若要了解设备可用存储空间、SSID 或 BSSID HoloLens请查看这些报表[IP。](#csp-changes-for-reporting-hololens-details) <br>
✔️ 如果要使用 WDAC 来阻止应用或进程启动，但还需要使用自己的一系列粘性应用，现在可以在 WDAC 策略 中允许[LOB。](#use-wdac-and-lob-apps)
...
对于排查行为问题时的托管设备，确认应用了预期的策略配置是一个重要步骤。 以前，对于此新功能，在导出通过 **设置** 帐户访问工作或学校收集的 MDM 诊断日志后，必须通过 MDM 或在设备附近查看此信息，然后选择"导出管理日志"，并查看附近的  ->    >  电脑。
...
- 更新了全新闪烁的远程协助的内置版本。
- 内部版本中禁用了2D 应用的游戏板处理。 通过删除应用程序，现在可以自由地直接使用游戏板 Api，并可以访问整个控件集，并执行所需的任何操作。 开发人员应使用游戏板 Api 来使用游戏板输入。 下面是[游戏板类 (Windows 的示例。游戏输入) Windows UWP 应用程序](/uwp/api/windows.gaming.input.gamepad?view=winrt-20348&preserve-view=true)
- 修复了第一次用户登录后，在使用基于 AAD 组的展台配置的方案中，OOBE 终止的问题。
...
![Windows内部通道说明。](images/WindowsInsiderChannels.png)
