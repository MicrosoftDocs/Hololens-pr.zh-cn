...
>[!NOTE]
> 我们很高兴再次开始向 Windows 预览体验成员提供新功能。 我们将测试到开发人员频道获取最新更新。 我们将继续使用 [**HoloLens 预览**](hololens-insider.md) 体验成员说明，因为我们向 Windows 预览体验成员版本添加更多功能和更新。 感到很高兴，并准备好将这些更新融合到你的现实中。
...
## Windows Holographic 版本 20H2 - 2021 年 2 月更新
- 内部版本 19041.1136
...
## Windows Holographic 版本 1903 - 2021 年 2 月更新
- 内部版本 18362.1098

此每月质量更新不包含任何值得注意的更改，我们鼓励你试用 Windows Holographic 版本 2004 的最新内部版本。

## Windows Holographic 版本 20H2 - 2021 年 1 月更新
- 内部版本 19041.1134
...
## Windows Holographic 版本 20H2 – 2020 年 12 月更新
- 内部版本 19041.1131

### 通过应用安装程序在 HoloLens 2 上安装应用

我们正在 **向应用安装程序 (** 新功能) 允许你在 HoloLens 2 设备上更加无缝地安装应用程序。 默认情况下，非 **托管设备将启用此功能**。 为了防止企业中断，应用安装程序目前对托管 **设备** 不可用。  
...
1.  导航到"下载"文件夹。 你可能需要在应用的左侧面板上先选择此设备，然后导航到"下载"。
...
- 手部跟踪现在在许多新情况下保持跟踪，这些新情况下手以前会丢失。  在某些新情况下，仅根据用户的实际手继续更新手部位置，而其他连接根据上一个位置进行推断。  此更改有助于提高移动（如跳跃、抛出、跳跃和包打）的跟踪一致性。  它还有助于在手靠近表面或按住对象的情况下。  在推断手部连接时 [，每个联合](https://docs.microsoft.com/uwp/api/windows.perception.people.jointposeaccuracy?view=winrt-19041&preserve-view=true) 精度值将设置为"近似"，而不是"高"。
- 修复了 Azure AD 帐户的 PIN 重置将显示错误"出错"的问题。
- 在启动 ET、设置应用中的 Iris、新用户或通知 Toast 时，用户应看到更少的启动后 OOBE 崩溃。
- 用户应具有来自 OOBE 的正确时区。

## Windows Holographic 版本 1903 – 2020 年 12 月更新
- 内部版本 18362.1088

此每月质量更新不包含任何明显的更改，我们鼓励你试用我们最新的 Windows 全息版版本 20H2 – 2020 年 12 月更新以及内部版本中添加的新应用安装程序功能。


## Windows 全息版版本 20H2
- 内部版本 19041.1128

Windows Holographic 版本 20H2 现已发布，为 HoloLens 2 用户和 IT 专业人员带来了一组出色的新功能。 从自动目视定位到"设置"中的证书管理器，到改进的展台模式功能和新的 Autopilot 设置功能。 此新更新使 IT 团队能够更精细地控制 HoloLens 设备的配置和管理，并为用户提供更无缝的全息体验。 

此最新版本是版本 2004 的每月更新，但这次我们将包含新功能。 主要内部版本号将保持不变，Windows 更新将指示版本 2004 的每月版本 (版本 19041) 。 可以在"设置">"关于"屏幕中查看内部版本号，以确认你已使用最新的可用内部版本 19041.1128+。 若要更新到最新版本，请打开"设置"应用，转到"&安全性"，然后点击"检查更新"。 若要详细了解如何管理 HoloLens 更新，请访问 [此页面](https://docs.microsoft.com/hololens/hololens-updates)。
...
| [Tenantlockdown 云解决方案提供商和 Autopilot](hololens-release-notes.md#tenantlockdown-csp-and-autopilot)                     | 在租户注册和应用策略后，设备每次重置或重新闪烁时，都只能在该租户中注册设备。 |
...
| [HoloLens 2 的"已启用设置"页面可见性](hololens-release-notes.md#enabled-settings-page-visibility-for-hololens-2)      | 用于选取"设置"应用中显示的页面的策略。             |
...
-   **诊断：** 出现问题时，验证设备上是否存在适当的证书可节省时间，并有助于进行故障排除。 
...
证书安装当前支持 .cer 和 .crt 文件。 设备所有者可以在本地计算机和当前用户中安装证书; 所有其他用户只能安装到当前用户中。 用户只能删除直接从"设置"UI 安装的证书。 如果证书已经通过其他方式安装，则还必须通过同一机制将其删除。
...
在此版本之前，用户必须手动在 OOBE 期间启动预配屏幕，才能使用按钮组合进行设置。 现在，用户可以通过使用 USB 存储驱动器上的预配包跳过按钮组合。 

1. 在 OOBE 第一次交互期间，将 USB 驱动器与预配包一起插入
...
你的设备现已配置， [将显示"预配成功"屏幕](hololens-provisioning.md#automatic-provisioning-without-using-ui)。

### 将 Autopilot Wi-Fi连接
- 通过使 Autopilot 在连接的设备上运行，无需 USB-C 适配器来减少Wi-Fi硬件需求。

现在，在 OOBE 期间，将 HoloLens 2 与 WLAN 连接后，OOBE 将检查设备的 Autopilot 配置文件。 如果找到一个，它将用于完成 AAD 加入和注册流的其余部分。 换句话说，不再要求使用以太网到 USB-C 或 Wi-Fi到 USB-C 适配器，但如果在 OOBE 开始时提供，它们将继续工作。 了解有关适用于 [HoloLens 2 设备的 Autopilot 的信息](hololens2-autopilot.md)。
...
### 全局分配的访问权限 – 展台模式
- 通过启用在系统级别应用展台模式的新 Kiosk 方法，减少展台的标识管理。
...
在应用展台模式失败之前，HoloLens 用于在"开始"菜单中显示所有应用程序。 现在，在 Windows 全息版 20H2 中，在出现故障的情况下，不会在"开始"菜单中显示应用，如下所示： 
...
| 新 HoloLens 策略                                | 描述                                                                               | 注释                                                                |
|----------------------------------------------------|-------------------------------------------------------------------------------------------|----------------------------------------------------------------------|
| MixedReality\BrightnessButtonDisabled              | 允许禁用亮度按钮，以便按它不会改变亮度。       | 1 是，0 (默认值)                                                 |
| MixedReality\VolumeButtonDisabled                  | 允许禁用音量按钮，以便按它不会改变音量。               | 1 是，0 (默认值)                                                 |
...
1. 对于任何其他 Azure AD 用户 N，可以重复步骤 4 和 5。此处的关键点是，任何 Azure AD 用户都必须使用 Internet 登录设备，以便我们至少一次可以确定他们是展台配置所面向的 Azure AD 组的成员。 
...
|     [EnergySaverBatteryThresholdOnBattery](https://docs.microsoft.com/windows/client-management/mdm/policy-csp-power#power-energysaverbatterythresholdonbattery)     |  用于 Windows 配置设计器的示例值，即 100                                                                             |
|     [EnergySaverBatteryThresholdPluggedIn](https://docs.microsoft.com/windows/client-management/mdm/policy-csp-power#power-energysaverbatterythresholdpluggedin)     |     用于 Windows 配置设计器的示例值，即 100                                                                          |
...
> [!NOTE]
> 如果发生以下情况之一 (HoloLens) 默认视频录制时长为 5 分钟：
...
## Windows Holographic 版本 1903 - 2020 年 11 月更新
- 内部版本 18362.1085
...
## Windows Holographic 版本 1903 - 2020 年 10 月更新
- 内部版本 18362.1081

此每月质量更新不包含任何值得注意的更改，我们鼓励你试用 Windows Holographic 版本 2004 的最新内部版本。

## Windows Holographic 版本 2004 - 2020 年 9 月更新
- 内部版本 19041.1117

更新中的改进和修复：

- 解决了当 appxmanifest Visual Studio SupportsMultipleInstances="true"时阻止用户调试应用程序的问题。
- 此版本包括 NCSI 代理检测修复，用于解决通过网络代理的 Internet 检测失败的问题。 NCSI 可以使用计算机代理和每个配置文件代理进行 Internet 连接检测。 在未来版本中，NCSI 将支持每用户代理。
...
## Windows Holographic 版本 1903 - 2020 年 9 月更新
- 内部版本 18362.1079
...
- 修复了在 OOBE 期间应用预配包以重命名设备并执行其他操作 (（如连接到网络) ）在设备重启后因重命名而无法执行其他操作的问题。
- 修改了初始设备设置流的配色方案，以提高视觉质量。

## Windows Holographic 版本 1903 - 2020 年 8 月更新
- 内部版本 18362.1074
...
- 修复了导致独占 HoloLens 应用在将 HoloLens 仿真器用于某些设备上硬件加速时崩溃回 shell 的问题。
- 解决了 HoloLens 2 上 Windows Device Portal 的 HTTPS 转发问题。
...
- 自定义 MRC 录制器现在具有某些属性的新默认值（如果未指定）。
  ...
  - 在 *MRC 音频效果上*：
    - LoopbackGain (Windows Device Portal) 的"混合现实捕获"页上的当前"应用音频获取") 
    - MicrophoneGain (Windows Device Portal) 的"混合现实捕获"页上的当前"麦克风音频获得") 
...
2020 年 5 月 HoloLens 2、Windows 全息版版本 *2004* 的主要软件更新包括一系列令人兴奋的新功能，如 Windows Autopilot 支持、应用深色模式、USB 以太网对 5G/LTE 热点的支持等。 若要更新到最新版本，请打开"设置****"应用，转到"&安全"，然后选择   "检查 **** **更新"**   按钮。 

|             功能                              |          描述                                                                                              |
|--------------------------------------------------|-------------------------------------------------------------------------------------------------------------------|
|       Windows Autopilot                          |          使用 Windows AutoPilot 预配置并无缝设置用于生产的新设备                 |
|       FIDO 2 支持                             |          支持 FIDO2 安全密钥，以支持对共享设备进行快速而安全的身份验证            |
|       改进的预配                      |          将预配包从 U 盘无缝应用到 HoloLens                              |
|       应用程序安装状态                 |          在"设置"应用中检查应用的安装状态已通过 MDM 推送到 HoloLens 2               |
...
适用于 HoloLens 2 的 Windows Autopilot 允许设备销售渠道将 HoloLens 预注册到 Intune 租户。 当设备到达时，他们已准备好在租户下自行部署为共享设备。 若要利用自部署，设备必须在设置的第一个屏幕期间使用 USB-C 到以太网连接到网络。

在用户启动 Autopilot 自部署过程后，此过程将完成以下步骤：
...
从适用于 [HoloLens 2 的 Windows Autopilot 评估指南中了解更多信息](https://docs.microsoft.com/hololens/hololens2-autopilot)。

*立即联系你的客户经理以加入 AutoPilot 预览版。 Autopilot 就绪设备将很快开始发货。*

### FIDO2 安全密钥支持

某些用户与工作或学校环境中其他人共享 HoloLens 设备。 因此，用户无需键入长用户名和密码即可轻松操作，这一点非常重要。 Fast Identity Online (FIDO) 可让组织 (Azure AD 租户) 无需输入用户名和密码即可无缝登录 HoloLens。

FIDO2 安全密钥是基于标准的无密码身份验证方法，可采用任何外形要求。 FIDO 是无密码身份验证的开放标准。 它允许用户和组织在没有用户名或密码的情况下登录其资源。 相反，它们使用内置于设备的外部安全密钥或平台密钥。
...
预配包让你可以通过 配置文件 而不是 HoloLens 开箱即用体验来设置 HoloLens 配置。 以前，预配包必须复制到 HoloLens 内部内存中。 现在，它们可以位于 USB 驱动器上，以便更轻松地在多个 HoloLens 设备上重复使用，并且你可以并行预配设备。 预配包现在还支持在设备管理中注册的字段，因此预配后无需手动设置。
...
策略配置服务提供程序使企业能够在 Windows 设备上配置策略。 在此版本中，我们添加了 HoloLens 的新策略，此处列出了这些策略。 若要了解更多信息，请参阅[HoloLens 2 支持的策略 CSP。](https://docs.microsoft.com/windows/client-management/mdm/policies-supported-by-hololens2)  
...
NetworkQoSPolicy 配置服务提供商通过 QoS 策略创建 (服务质量) 策略。 QoS 策略根据一组匹配的条件对网络流量执行一组操作。 若要了解更多信息，请参阅[NetworkQoSPolicy CSP。](https://docs.microsoft.com/windows/client-management/mdm/networkqospolicy-csp)

### 扩展了对 5G/LTE 连接设备的 USB 以太网支持

添加了支持以启用某些移动宽带设备，例如 5G/LTE 电话和 Wi-Fi 热点，当它们通过 USB 连接到 HoloLens 2 时。 这些设备现在在网络设置 **中显示为** 另一个以太网连接。  (不支持需要外部驱动程序的移动宽带设备。) 如果 Wi-Fi 不可用并且 Wi-Fi连接性能不足，此功能将启用高带宽连接。 若要了解有关受支持的 USB 设备的信息，请参阅Bluetooth [和 USB-C 设备](https://docs.microsoft.com/hololens/hololens-connect-devices)。  
...
- **减少意外的点击：** 我们改进了对空点击手势的检测。 现在，在一些常见方案中（例如，当你将手放在一边时）意外激活更少。
- **用户交换机可靠性：** 现在，在共享设备时，系统更新手型的速度更快且更可靠。
- **减少手盗：** 我们改进了对传感器视图多于两个手的情况的处理。 如果多个人紧密协作，则跟踪手从用户"跳"到场景中其他人手的可能性要低得多。
- **系统可靠性：** 修复了导致手部跟踪在设备负载较高时停止工作的问题。

### 深色模式

许多 Windows 应用现在支持深色和浅色模式。 HoloLens 2 用户可以为支持两者的应用选择默认模式。 更新后，默认应用模式为"深色"，但你可以轻松更改此设置：**导航到"** 设置  >  **系统**  >  **颜色**  >  **"选择默认应用模式**。 
...
更新中的改进和修复：  
- 引入了活动显示校准系统。 此功能改进了全息影像的稳定性和对齐方式。 当你将头从一侧移动到另一侧时，它们现在就位。
...
- 问题需要你在启动设备后启动 Cortana 应用以使用"你好小娜"语音激活。 如果你从 18362 版本更新，你也可能看到以前版本的 Cortana 应用的第二个应用磁贴，该磁贴在"开始"版本中不再 **起作用**。
...
许多 Windows 应用支持深色和浅色模式。 HoloLens 2 客户现在可以为支持这两种配色方案的应用选择默认模式。 根据客户反馈，我们将默认应用模式设置为"深色"，但你随时可以轻松地更改此设置：导航到"设置 **">系统 > 颜色**以查找"**选择默认**应用模式"。
...
## Windows Holographic 版本 1903 - 2020 年 2 月更新 
- 内部版本 18362.1053
...
## Windows Holographic 版本 1903 - 2020 年 1 月更新 
- 内部版本 18362.1043
...
## Windows Holographic 版本 1903 - 2019 年 11 月更新 
- 内部版本 18362.1039

更新中的改进和修复：

- 修复了在初始**** 设置 en-CA 和 en-AU 期间选择语音命令的功能。
- 改进了在 MRTK 版本的最新 Unity 和混合现实中Toolkit (对象) 质量。
